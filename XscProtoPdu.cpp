...
shared_ptr<XscProtoPdu> XscProtoPdu::decode(uchar* dat, int len)
{
	bool exp;
	return XscProtoPdu::decode(dat, len, &exp);
}
...
uchar* XscProtoPdu::bytes(int* len)
{
	...
	if (lenlen == 1)
	{
		ofst = XSC_TRANSM_MTU - sizeh - sizet - 2 ;
		__pdu_bytes__[ofst] = this->transm.header == NULL ? 0x00 : 0x01 ;
		__pdu_bytes__[ofst + 1] = (uchar) size;
	} else if (lenlen == 2)
	{
	...
	{
		ofst = XSC_TRANSM_MTU - sizeh - sizet - 4 ;
		__pdu_bytes__[ofst] = this->transm.header == NULL ? 0x04 : 0x05 ;
		uint x = ::htonl(size);
		::memcpy(__pdu_bytes__ + ofst + 1, ((uchar*) &x) + 1, 3);
	} else if (lenlen == 4)
	{
		ofst = XSC_TRANSM_MTU - sizeh - sizet - 5 ;
		__pdu_bytes__[ofst] = this->transm.header == NULL ? 0x06 : 0x07 ;
		uint x = ::htonl(size);
		::memcpy(__pdu_bytes__ + ofst + 1, ((uchar*) &x), 4);
	} else
	{
		LOG_FAULT("it`s a bug, lenlen: %d", lenlen)
	}
	*len = size;
	return __pdu_bytes__ + ofst;
}
