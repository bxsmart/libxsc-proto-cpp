...
bool XscProtoTransaction::decodeContinue(xsc_tlv_node* root)
{
	LOG_DEBUG("unsupported XSC_TAG_TRANS_CONTINUE")
	return false;
}

bool XscProtoTransaction::decodeDialog(xsc_tlv_node* root)
{
	LOG_DEBUG("unsupported XSC_TAG_TRANS_DIALOG")
	return false;
}

bool XscProtoTransaction::decodeCancel(xsc_tlv_node* root)
{
	LOG_DEBUG("unsupported XSC_TAG_TRANS_CANCEL")
	return false;
}

bool XscProtoTransaction::decodeAbort(xsc_tlv_node* root)
{
...
bool XscProtoTransaction::decodeUnidirection(xsc_tlv_node* root)
{
	...
	if (XscProtoDec::getChar(root, XSC_TAG_HAVE_NEXT_PART, &part) == 0 && part == PRESENT)
	{
		LOG_DEBUG("it`s a bug, incomplete")
		return false;
	}
	if (msg != NULL)
		this->msg.assign((char*) msg, msglen);
...
{
	XscProtoTransaction::encodeUnidirection(trans->msg, trans->dat, trans->dlen, xtp);
}

void XscProtoTransaction::encodeUnidirection(const string& msg, uchar* dat, int len, xsc_tlv_pdu* xtp)
{
	if (len > 0)
		XscProtoEnc::addBin(xtp, XSC_TAG_DAT, dat, len);
	XscProtoEnc::addBin(xtp, XSC_TAG_MSG, (uchar*) msg.data(), msg.length());
...
void XscProtoTransaction::cloneDat()
{
	this->refDat = false;
	uchar* dat = (uchar*) ::malloc(this->dlen);
	::memcpy(dat, this->dat, this->dlen);
	this->dat = dat;
}
